    ...
    return (_host, _hostContext) => {
        ...
        return schematics_1.chain([
            ...
            schematics_1.mergeWith(schematics_1.apply(schematics_1.empty(), [
                ...
                // }),
                (tree, _context) => {
                    NgFileSystem_1.setVirtual(tree);
                    // extend project entry point:
                    return schematics_1.schematic("app-projects", projectOptions);
                },
                (tree, _context) => {
                    // extend project entry point:
                ...
                (_tree, _context) => {
                    return schematics_1.move(options.name);
                }
            ]), schematics_1.MergeStrategy.Overwrite),
            (tree, context) => {
                const installChain = [];
                if (!options.skipInstall) {
                    const installTask = context.addTask(new tasks_1.NodePackageInstallTask(options.name));
